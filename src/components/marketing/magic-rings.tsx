...
// Adapted from the React Bits "MagicRings" component
// (https://reactbits.dev) — a WebGL shader of concentric pulsing rings.
// Loaded client-only (see magic-rings-background.tsx) so the three.js
// bundle never blocks the hero's first paint, and only mounted when
...
export default function MagicRings({
  ...
  // Kept on a ref (not state) since the render loop below reads it every
  // frame from inside a rAF callback, not from React's render output —
  // assigning it in an effect (runs after every render) keeps refs out of
  // the render body itself while still being current before the next frame.
  useEffect(() => {
    propsRef.current = {
      color,
      colorTwo,
  ...
  useEffect(() => {
    ...
    // Paint one frame synchronously instead of waiting for the first rAF
    // tick, so there's no blank flash before the loop below kicks in (and
    // so a backgrounded/non-visible tab — where rAF is browser-throttled —
    // still shows a static frame rather than nothing).
    const initialProps = propsRef.current;
    if (initialProps) {
      uniforms.uColor.value.set(initialProps.color!);
      uniforms.uColorTwo.value.set(initialProps.colorTwo!);
      uniforms.uAttenuation.value = initialProps.attenuation!;
      uniforms.uLineThickness.value = initialProps.lineThickness!;
      uniforms.uBaseRadius.value = initialProps.baseRadius!;
      uniforms.uRadiusStep.value = initialProps.radiusStep!;
      uniforms.uScaleRate.value = initialProps.scaleRate!;
      uniforms.uRingCount.value = initialProps.ringCount!;
      uniforms.uOpacity.value = initialProps.opacity!;
      uniforms.uNoiseAmount.value = initialProps.noiseAmount!;
      uniforms.uRotation.value = (initialProps.rotation! * Math.PI) / 180;
      uniforms.uRingGap.value = initialProps.ringGap!;
      uniforms.uFadeIn.value = initialProps.fadeIn!;
      uniforms.uFadeOut.value = initialProps.fadeOut!;
      uniforms.uHoverScale.value = initialProps.hoverScale!;
